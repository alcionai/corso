...
func prepNewTestBackupOp(
	...
	*m365.Controller,
	...
	ctrl, sel := NewControllerWithSelector(t, ctx, acct, rsc, sel, nil, closer)
...
func testExchangeContinuousBackups(suite *BackupOpIntegrationSuite, toggles control.Toggles) {
	...
	ctrl, sels := NewControllerWithSelector(t, ctx, acct, resource.Users, sel.Selector, nil, nil)
...
func runDriveIncrementalTest(
	...
	ctrl, sel := NewControllerWithSelector(t, ctx, acct, resourceCat, sel, nil, nil)
	ac := ctrl.AC.Drives()
	...
	for _, test := range table {
		suite.Run(test.name, func() {
			cleanGC, err := m365.NewController(ctx, acct, resourceCat)
			require.NoError(t, err, clues.ToCore(err))
