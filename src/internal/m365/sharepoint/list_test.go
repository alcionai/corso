...
type ListIntgSuite struct {
	tester.Suite
	creds account.M365Config
}

func (suite *ListIntgSuite) SetupSuite() {
	t := suite.T()
...
func TestListIntgSuite(t *testing.T) {
	suite.Run(t, &ListIntgSuite{
...
func (suite *ListIntgSuite) TestLoadList() {
	t := suite.T()
...
type ListInfoUnitSuite struct {
	tester.Suite
}

func TestListInfoUnitSuite(t *testing.T) {
	suite.Run(t, &ListInfoUnitSuite{Suite: tester.NewUnitSuite(t)})
}

func (suite *ListInfoUnitSuite) TestSharePointInfo() {
	tests := []struct {
		name         string
