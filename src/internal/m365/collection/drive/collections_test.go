...
func (suite *OneDriveCollectionsUnitSuite) TestPopulateDriveCollections() {
	...
	tests := []struct {
		...
		},
		{
			...
			items: []models.DriveItemable{
				...
				driveItem("fileInFolder", "fileInFolder", testBaseDrivePath+folder, "folder", true, false, false),
				driveItem(
					"fileInFolder2", "fileInFolder2", testBaseDrivePath+folder+subFolder+folder, "folder2", true, false, false,
				),
		...
		},
		{
			...
			items: []models.DriveItemable{
				...
				driveItem("fileInFolder", "fileInFolder", testBaseDrivePath+folder, "folder", true, false, false),
				driveItem(
					"fileInSubfolder", "fileInSubfolder", testBaseDrivePath+folder+subFolder, "subfolder", true, false, false,
				),
				driveItem("fileInPackage", "fileInPackage", testBaseDrivePath+pkg, "package", true, false, false),
			},
			inputFolderMap:   map[string]string{},
	...
	for _, test := range tests {
		suite.Run(test.name, func() {
			...
			assert.Equal(t, test.expectedCountPackages, countPackages, "count of collectsion marked as packages")
		})
	}
}
