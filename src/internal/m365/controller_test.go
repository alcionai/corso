...
func (suite *ControllerUnitSuite) TestController_Status() {
	t := suite.T()
...
func (suite *ControllerUnitSuite) TestVerifyBackupInputs_allServices() {
...
type ControllerIntgSuite struct {
	tester.Suite
	ctrl          *Controller
	user          string
...
func TestControllerIntgSuite(t *testing.T) {
	suite.Run(t, &ControllerIntgSuite{
...
func (suite *ControllerIntgSuite) SetupSuite() {
	t := suite.T()
...
func (suite *ControllerIntgSuite) TestRestoreFailsBadService() {
	t := suite.T()
...
func (suite *ControllerIntgSuite) TestEmptyCollections() {
	dest := tester.DefaultTestRestoreDestination("")
	table := []struct {
		name string
...
func runRestore(
	...
	restoreGC := newController(ctx, t, config.Resource)
...
func runBackupAndCompare(
	...
	backupGC := newController(ctx, t, config.Resource)
...
func (suite *ControllerIntgSuite) TestRestoreAndBackup() {
...
func (suite *ControllerIntgSuite) TestMultiFolderBackupDifferentNames() {
	table := []restoreBackupInfo{
		{
			name:        "Contacts",
			service:     path.ExchangeService,
	...
	for _, test := range table {
		suite.Run(test.name, func() {
			...
			for i, collection := range test.collections {
				...
				restoreGC := newController(ctx, t, test.resourceCat)
				deets, err := restoreGC.ConsumeRestoreCollections(
					ctx,
					version.Backup,
					restoreSel,
			...
			backupGC := newController(ctx, t, test.resourceCat)
...
func (suite *ControllerIntgSuite) TestRestoreAndBackup_largeMailAttachment() {
...
func (suite *ControllerIntgSuite) TestBackup_CreatesPrefixCollections() {
	table := []struct {
		name         string
	...
	for _, test := range table {
		suite.Run(test.name, func() {
			...
			var (
				backupGC  = newController(ctx, t, test.resource)
