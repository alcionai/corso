...
func (suite *BackupIntgSuite) TestExchangeBackup() {
	...
	for _, test := range tests {
		for _, canMakeDeltaQueries := range []bool{true, false} {
			...
			suite.Run(name, func() {
				...
				collections, excludes, err := exchange.ProduceBackupCollections(
					ctx,
					suite.ac,
					sel,
					suite.tenantID,
...
func (suite *BackupIntgSuite) TestBackup_invalidResourceOwner() {
	ctx, flush := tester.NewContext(suite.T())
	defer flush()
	...
	for _, test := range tests {
		suite.Run(test.name, func() {
			...
			collections, excludes, err := ctrl.ProduceBackupCollections(
				ctx,
				test.getSelector(t),
				test.getSelector(t),
				nil,
...
func (suite *BackupIntgSuite) TestSharePointBackup() {
	ctx, flush := tester.NewContext(suite.T())
	defer flush()
	...
	for _, test := range tests {
		suite.Run(test.name, func() {
			...
			collections, excludes, err := sharepoint.ProduceBackupCollections(
				ctx,
				suite.ac,
				sel,
				sel,
...
func (suite *SPCollectionIntgSuite) SetupSuite() {
	...
	suite.ctrl = newController(ctx, suite.T(), resource.Sites)
...
func (suite *SPCollectionIntgSuite) TestCreateSharePointCollection_Libraries() {
	...
	cols, excludes, err := ctrl.ProduceBackupCollections(
		ctx,
