...
// these rules:
//  1. If the called BackupBases has an entry for a given reaon, ignore the
//     passed in BackupBases for that reason.
//  2. If the the called BackupBases has only AssistBases, look for a matching
...
func (bb *backupBases) fixupAndVerify(ctx context.Context) {
	toDrop := findNonuniqeManifests(ctx, bb.mergeBases)
