...
func (suite *KopiaIntegrationSuite) TestBackupCollections() {
	...
	runAndTestBackup := func(test testCase, base ManifestEntry) ManifestEntry {
		...
		suite.Run(test.name, func() {
			...
			assert.Equal(t, test.expectedUploadedFiles, counter.Get(count.PersistedFiles), "total files")
			assert.Equal(t, test.expectedUploadedFiles, stats.UncachedFileCount, "uncached files")
			assert.Equal(t, test.expectedUploadedFiles, counter.Get(count.PersistedNonCachedFiles), "uncached files")
			assert.Equal(t, test.expectedCachedFiles, stats.CachedFileCount, "cached files")
			assert.Equal(t, test.expectedCachedFiles, counter.Get(count.PersistedCachedFiles), "cached files")
			assert.Equal(t, 4+len(test.collections), stats.TotalDirectoryCount, "directory count")
			assert.Equal(t, 4+len(test.collections), counter.Get(count.PersistedDirectories), "directory count")
...
func (suite *KopiaIntegrationSuite) TestBackupCollections_NoDetailsForMeta() {
	...
	for _, test := range table {
		suite.Run(test.name, func() {
			...
			assert.Equal(t, 5, counter.Get(count.PersistedDirectories), "uploaded directories")
			assert.Zero(t, stats.IgnoredErrorCount, "ignored errors")
			assert.Zero(t, counter.Get(count.PersistenceIgnoredErrors), "ignored errors")
			assert.Zero(t, stats.ErrorCount, "errors")
			assert.Zero(t, counter.Get(count.PersistenceErrors), "errors")
...
func (suite *KopiaIntegrationSuite) TestBackupCollections_ReaderError() {
	...
	assert.Equal(t, 5, counter.Get(count.PersistedFiles), "total files")
	assert.Equal(t, 6, stats.TotalDirectoryCount, "total directories")
	assert.Equal(t, 6, counter.Get(count.PersistedDirectories), "total directories")
	assert.Zero(t, stats.IgnoredErrorCount, "ignored errors")
	assert.Zero(t, counter.Get(count.PersistenceIgnoredErrors), "ignored errors")
...
func (suite *KopiaSimpleRepoIntegrationSuite) SetupTest() {
	...
	require.Equal(t, expectedFiles, counter.Get(count.PersistedFiles))
	require.Equal(t, expectedDirs, stats.TotalDirectoryCount)
	require.Equal(t, expectedFiles, counter.Get(count.PersistedDirectories))
...
func (suite *KopiaSimpleRepoIntegrationSuite) TestBackupExcludeItem() {
	...
	for _, test := range table {
		suite.Run(test.name, func() {
			...
			assert.Equal(t, test.expectedCachedItems, counter.Get(count.PersistedCachedFiles))
			assert.Equal(t, test.expectedUncachedItems, counter.Get(count.PersistedNonCachedFiles))
