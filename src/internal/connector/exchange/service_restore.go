...
// are displayed in a flat representation. Therefore, only the root can
// be populated and all content must be restored into the root location.
...
func establishEventsRestoreLocation(
	...
	if isNewCache {
		...
		transform := graph.CreateCalendarDisplayable(temp)
