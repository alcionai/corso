...
A Corso [repository](/concepts#corso-concepts) stores encrypted copies of your backup data. Repositories are currently
...
:::note
Depending on community interest, support for other object storage backends may be added in the future.
:::
...
* S3 bucket for the repo has already been created. Corso will not create it for you.
* You have access to credentials for a user or an IAM role that confer the following permissions
...
Corso supports the credential options offered by the Go SDK. Full details can be found in the *Specifying Credentials*
...
* **Credentials file** - ensure that the credentials file is available to Corso (e.g. may need to map it if using Corso
  as a container). Additionally, you may want to set and export `AWS_PROFILE`, if not using the default profile, and
...
Before first use, you need to initialize a Corso repositoy with `corso repo init s3`. See command details
